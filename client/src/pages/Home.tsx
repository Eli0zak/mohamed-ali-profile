...
const proofCategories: ProofCategory[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { title: "Training Room Story", titleAr: "مشهد من قاعة تدريب", organization: "Training evidence", organizationAr: "دليل تدريب", type: "Event / training moment", typeAr: "لحظة فعالية / تدريب", description: "A cinematic room view that shows the scale and atmosphere of live learning moments.", descriptionAr: "مشهد تحريري من قاعة يوضح حجم وأجواء لحظات التعلم المباشر.", image: "/training-assets/WhatsApp Image 2026-08-12 at 8.15.29 PM (3).jpeg", alt: "Training room with a live audience", caption: "A live room built for team learning and practice.", captionAr: "قاعة حية مهيأة لتعلم الفريق والتطبيق العملي." },
      { title: "180 Daraga — Main Stage", titleAr: "180 Daraga — المسرح الرئيسي", organization: "180 Daraga", organizationAr: "180 Daraga", type: "Speaking engagement", typeAr: "مشاركة كمتحدث", description: "A live speaking moment at 180 Daraga, presenting with a microphone in front of the event audience and visual identity.", descriptionAr: "لحظة تحدث مباشرة في فعالية 180 Daraga، يظهر فيها محمد بالميكروفون أمام الحضور وهوية الفعالية.", image: "/manus-storage/180-daraga-cover_1b4889af.webp", alt: "Mohamed Ali speaking on stage at 180 Daraga", caption: "Sharing practical sales and commercial lessons with a live audience.", captionAr: "مشاركة خبرات عملية في المبيعات والتجارة أمام جمهور مباشر." },
      { title: "180 Daraga — MEC Academy", titleAr: "180 Daraga — MEC Academy", organization: "MEC Academy", organizationAr: "MEC Academy", type: "Partnership evidence", typeAr: "دليل شراكة", description: "A stage view connecting the speaking engagement with MEC Academy and the event sponsor context.", descriptionAr: "لقطة من المسرح تربط المشاركة كمتحدث بـMEC Academy وسياق دعم الفعالية.", image: "/manus-storage/180-daraga-mec-screen_68c38281.webp", alt: "MEC Academy sponsor visual during 180 Daraga" },
      { title: "180 Daraga — Community Moment", titleAr: "180 Daraga — تفاعل مجتمعي", organization: "180 Daraga", organizationAr: "180 Daraga", type: "Community engagement", typeAr: "تفاعل مجتمعي", description: "A community moment with participants after the event, showing direct engagement beyond the stage.", descriptionAr: "لحظة تفاعل مجتمعي مع المشاركين بعد الفعالية، توضح التواصل المباشر خارج المسرح.", image: "/manus-storage/180-daraga-community_3355947c.jpg", alt: "Mohamed Ali with participants at 180 Daraga" },
      { title: "180 Daraga — Speaker Portrait", titleAr: "180 Daraga — صورة المتحدث", organization: "180 Daraga", organizationAr: "180 Daraga", type: "Speaking moment", typeAr: "لحظة تحدث", description: "A focused speaker portrait capturing Mohamed with the 180 Daraga event identity and microphone in frame.", descriptionAr: "صورة مركزة لمحمد كمتحدث مع ظهور هوية فعالية 180 Daraga والميكروفون في المشهد.", image: "/manus-storage/180-daraga-stage-dark_b7cad0d4.webp", alt: "Mohamed Ali presenting at 180 Daraga" },
    ],
  },
];
...
const homeCopy = {
  en: {
    hero: {
      ...
      role: "Business Development / Sales Leadership / Training & Commercial Operations",
      lede: "I build commercial teams, improve sales operations, and turn real-world experience into practical training across Egypt's education and training sector.",
  ...
  ar: {
    hero: {
      ...
      role: "تطوير الأعمال / قيادة المبيعات / التدريب والعمليات التجارية",
      lede: "أبني فرقًا تجارية أقوى، وأطوّر عمليات المبيعات، وأحوّل الخبرة العملية إلى تدريب قابل للتطبيق في قطاع التعليم والتدريب بمصر.",
...
export default function Home() {
  ...
  const handleTimelineKeyDown = (event: React.KeyboardEvent<HTMLButtonElement>, index: number) => {
    ...
    event.preventDefault();
    selectTimelineStation(nextIndex, true);
  };
        ...
        <section className="hero-section hero-section--motion" style={{ "--orbit-one-x": `${scrollProgress * -12}px`, "--orbit-one-y": `${scrollProgress * 10}px`, "--orbit-two-x": `${scrollProgress * 14}px`, "--orbit-two-y": `${scrollProgress * -8}px`, backgroundImage: `linear-gradient(90deg, rgba(3,7,18,.98) 0%, rgba(3,7,18,.82) 45%, rgba(3,7,18,.25) 100%), url("${asset.hero}")` } as React.CSSProperties}>
          ...
          <div className="container hero-grid">
            ...
            <div className="hero-portrait-wrap hero-portrait-wrap--motion" data-reveal style={{ "--parallax-y": `${scrollProgress * -18}px` } as React.CSSProperties}>
              ...
              <div className="portrait-card"><img src={asset.profile} alt="Mohamed Ali presenting a training session" loading="eager" decoding="async" fetchPriority="high" /><div className="portrait-card__label"><span>MOHAMED ALI</span><small>BUSINESS DEVELOPMENT · EGYPT</small></div></div>
        ...
        <section id="journey" className="journey-section career-route-section">
          <div className="container">
            ...
            <div className="career-route" dir={language === "ar" ? "rtl" : "ltr"}>
                ...
                <div className="career-route__stations" role="list">
                  {timeline.map((item, index) => {
                    ...
                    return <button ref={(element) => { careerStationRefs.current[index] = element; }} className={`career-station ${active ? "career-station--active" : ""} ${isSource || isTarget ? "career-station--handoff" : ""} ${isDocking ? "career-station--docking" : ""} ${index < selectedTimelineIndex ? "career-station--visited" : ""}`} type="button" role="listitem" key={item.company} onClick={() => selectTimelineStation(index)} onKeyDown={(event) => handleTimelineKeyDown(event, index)} onMouseEnter={() => setHoveredTimelineIndex(index)} onMouseLeave={() => setHoveredTimelineIndex(null)} onFocus={() => setHoveredTimelineIndex(index)} onBlur={() => setHoveredTimelineIndex(null)} style={{ left: stationPosition.left, top: stationPosition.top }} aria-pressed={active} aria-current={active ? "step" : undefined} aria-describedby="career-route-keyboard-help" aria-keyshortcuts="ArrowLeft ArrowRight ArrowUp ArrowDown Home End" tabIndex={active ? 0 : -1} aria-label={`${language === "ar" ? "فتح محطة" : "Open station"}: ${item.company}`}>
                      ...
                      <span className="career-station__logo"><img src={item.logo} alt="" loading="lazy" decoding="async" fetchPriority="low" onError={(event) => { event.currentTarget.src = "/brand-mark.svg"; }} /></span>
              ...
              <div className="career-route__progress" aria-label={language === "ar" ? "نسبة التقدم في الرحلة المهنية" : "Career journey progress"}>
                <div className="career-route__progress-head"><span>{language === "ar" ? "التقدم في المسار" : "Route progress"}</span><strong>{routeProgressPercent}%</strong></div>
                <div className="career-route__progress-track" role="progressbar" aria-valuemin={0} aria-valuemax={100} aria-valuenow={routeProgressPercent} aria-valuetext={language === "ar" ? `${routeProgressPercent}% من الرحلة المهنية` : `${routeProgressPercent}% of the career route`}>
                  <span className="career-route__progress-fill" style={{ width: `${routeProgressPercent}%` }} />
                  <span className="career-route__progress-glow" style={{ left: `${routeProgressPercent}%` }} aria-hidden="true" />
                  {routeStationProgress.map((progress, index) => <span key={`${timeline[index].company}-progress`} className={`career-route__progress-tick ${index <= selectedTimelineIndex ? "career-route__progress-tick--active" : ""}`} style={{ left: `${progress * 100}%` }} aria-hidden="true" />)}
                </div>
                <div className="career-route__progress-meta"><span>01 · {timeline[0].company}</span><span>{String(timeline.length).padStart(2, "0")} · {timeline[timeline.length - 1].company}</span></div>
              </div>
